#!/usr/bin/env python3
"""Congestion-aware DMPC controller with occupancy-based congestion cost."""
...
OccupancyMap = Dict[int, Dict[Cell, float]]
...
class CADMPCController(Node):
    """Local CA-DMPC controller with occupancy-based congestion penalty."""

    def __init__(self) -> None:
        super().__init__('ca_dmpc_controller')

        self.declare_parameter('robot_name', '')
        self.declare_parameter('path_topic', '/plan')
        self.declare_parameter('odom_topic', '/odom')
        self.declare_parameter('neighbor_topic', '/neighbor_trajectories')
        self.declare_parameter('occupancy_topic', '/occupancy_debug')
        ...
        self.declare_parameter('wc', 30.0)
        ...
        self._occupancy_topic = normalize_topic(str(self.get_parameter('occupancy_topic').value))
        ...
        self._wc = max(0.0, float(self.get_parameter('wc').value))
        ...
        self._occupancy_stale_after_sec = max(2.0 * self._control_period, 1.0)

        self._path_points: List[Point2D] = []
        self._current_state: Optional[State] = None
        self._neighbor_trajectories: List[NeighborTrajectory] = []
        self._occupancy_by_time: OccupancyMap = {}
        self._occupancy_grid_resolution: Optional[float] = None
        self._last_occupancy_update_ns: Optional[int] = None
        ...
        self._occupancy_sub = self.create_subscription(
            String,
    ...
    def _occupancy_callback(self, msg: String) -> None:
        self._occupancy_by_time, self._occupancy_grid_resolution = self._parse_occupancy_payload(msg.data)
        self._last_occupancy_update_ns = self.get_clock().now().nanoseconds
    ...
    def _parse_occupancy_payload(self, raw_json: str) -> Tuple[OccupancyMap, Optional[float]]:
        try:
            payload = json.loads(raw_json)
        except json.JSONDecodeError:
            self.get_logger().warn('Failed to parse occupancy JSON; using zero congestion cost.')
            return {}, None
        ...
        try:
            grid_resolution = float(payload.get('grid_resolution', 0.0))
        except (TypeError, ValueError):
            return {}, None
        ...
        occupancy_by_time: OccupancyMap = {}
        for entry in occupancy:
            ...
            values_by_cell: Dict[Cell, float] = {}
            for cell in cells:
                if not isinstance(cell, dict):
                    continue
                try:
                    ix = int(cell.get('ix', 0))
                    iy = int(cell.get('iy', 0))
                    value = float(cell.get('value', 0.0))
                except (TypeError, ValueError):
                    continue
                if value <= 0.0:
                    continue
                values_by_cell[(ix, iy)] = values_by_cell.get((ix, iy), 0.0) + value

            if values_by_cell:
                occupancy_by_time[t_index] = values_by_cell

        return occupancy_by_time, grid_resolution
    ...
    @staticmethod
    def _to_cell(x: float, y: float, grid_resolution: float) -> Cell:
        ix = int(math.floor(x / grid_resolution))
        iy = int(math.floor(y / grid_resolution))
        return (ix, iy)

    def _is_occupancy_stale(self) -> bool:
        if self._last_occupancy_update_ns is None:
            return True
        now_ns = self.get_clock().now().nanoseconds
        age_sec = (now_ns - self._last_occupancy_update_ns) * 1e-9
        return age_sec > self._occupancy_stale_after_sec
    ...
    def _objective(self, controls_flat: Sequence[float], initial_state: State, reference: Sequence[Reference]) -> float:
        ...
        occupancy_enabled = (
            not self._is_occupancy_stale()
            and self._occupancy_grid_resolution is not None
            and self._occupancy_grid_resolution > 0.0
        )
        occupancy_resolution = self._occupancy_grid_resolution if occupancy_enabled else None
        occupancy_by_time = self._occupancy_by_time if occupancy_enabled else {}
        ...
        for k, (x, y, yaw, v, w) in enumerate(rollout):
            ...
            # Congestion penalty from sparse occupancy map.
            if occupancy_resolution is not None:
                cells_at_k = occupancy_by_time.get(k)
                if cells_at_k:
                    ix, iy = self._to_cell(x, y, occupancy_resolution)
                    occupancy_value = cells_at_k.get((ix, iy), 0.0)
                    if occupancy_value > 0.0:
                        cost += self._wc * occupancy_value
